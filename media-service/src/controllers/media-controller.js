...
const getAllMedias = async (req, res) => {
  ...
  try {
    console.log("Testinggg", req.user.userId);
     const result =  await Media.find({userId : req.user.userId});
     console.log("result", result);
     logger.info(`Found ${result.length} medias for user ${req.user.userId}`);

      if(result.length ===0){
          logger.warn("No medias found for user", req.user.userId);
